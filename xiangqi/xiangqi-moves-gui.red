Red [
	"Move generation for GUI"
	filename: %xiangqi-moves-gui.red
	author:   "Arnold van Hofwegen"
	version:  0.5.4
	date:     "23-Nov-2015"
	needs:	  {xiangqi-move-common.red, Red 0.6.0 or GUI branch}
	info: { This is a stub for xiangqi-moves.red for getting the moves without the need 
	for the hash values and evaluation results, speeding up compilation speed
	and reducing complexity in the GUI test phase.
	}
]
; Added element-in-collection
	...
	if valid-move [
		...
		if any [	all [	not tam-capture
			...
			if tam-calculate-hash [
				new-board-hash-value: 0 ;; calculate-new-hash-from-move in-board get-hash m n
				append/only this-move new-board-hash-value
			]
			if not tam-skip-values [
				board-change-value: 0 ;; evaluate-move-value reduce [piece-value m n captured]
